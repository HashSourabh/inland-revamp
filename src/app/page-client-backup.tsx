...
// --- RESILIENT API UTILITY ---
/**
 * Creates a fetch request with timeout and abort controller
 */
function createFetchWithTimeout(url: string, timeoutMs: number = 15000): Promise<Response> {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
  
  return fetch(url, {
    signal: controller.signal,
    // Let browser handle caching based on server response headers
    // Don't send cache-control header to avoid CORS preflight issues
    cache: 'default',
  }).finally(() => {
...
const propertyService = {
  async getFeaturedProperties(page = 1, pageSize = 9): Promise<DatabaseProperty[]> {
    try {
      ...
      const res = await retryWithBackoff(() => 
        createFetchWithTimeout(url, 15000) // 15 second timeout
      );
      ...
      if (err.name === 'AbortError') {
        console.error("Featured properties API request timed out after 15 seconds");
      } else {
  ...
  async getExclusiveProperties(): Promise<DatabaseProperty[]> {
    try {
      ...
      if (err.name === 'AbortError') {
        console.error("Exclusive properties API request timed out after 15 seconds");
      } else {
  ...
  async getPropertyTypes(): Promise<PropertyType[]> {
    try {
      ...
      const res = await retryWithBackoff(() => 
        createFetchWithTimeout(url, 10000) // 10 second timeout (shorter for types)
      );
      ...
      if (err.name === 'AbortError') {
        console.error("Property types API request timed out after 10 seconds");
      } else {
...
export default function Home() {
  ...
  // Store raw property data for retransformation when types arrive
  const rawExclusiveProperties = useRef<DatabaseProperty[]>([]);
  const rawFeaturedProperties = useRef<DatabaseProperty[]>([]);

  // Load property types (can load in parallel with properties)
  useEffect(() => {
    ...
    const loadPropertyTypes = async () => {
      try {
        ...
        // Retransform properties if we have raw data waiting
        if (rawExclusiveProperties.current.length > 0) {
          const transformed = rawExclusiveProperties.current.map(p => 
            transformPropertyForCard(p, typesMap, tCommon)
          );
          setExclusiveProperties(transformed);
          rawExclusiveProperties.current = [];
        }
        if (rawFeaturedProperties.current.length > 0) {
          const transformed = rawFeaturedProperties.current.map(p => 
            transformPropertyForCard(p, typesMap, tCommon)
          );
          setFeaturedProperties(transformed);
          rawFeaturedProperties.current = [];
        }
      } catch (err) {
  ...
  // Load exclusive properties - with stale-while-revalidate and parallel loading
  useEffect(() => {
    const hasCachedData = exclusiveProperties.length > 0;
    const isStale = needsRefresh;
    
    // Stale-while-revalidate: If we have cached data (even if stale), show it immediately
    if (hasCachedData) {
      setExclusiveLoading(false);
      hasLoadedOnce.current = true;
      
      // If data is stale, fetch fresh data in background (don't show loading)
      if (isStale) {
        console.log('Exclusive properties cache is stale, fetching fresh data in background...');
    ...
    const loadExclusive = async () => {
      try {
        ...
        if (!hasCachedData) {
          console.log('Loading exclusive properties...');
          setExclusiveLoading(true);
        } else {
          console.log('Refreshing exclusive properties in background...');
        }
        ...
        // Check if types are available for transformation
        const hasTypes = Object.keys(propertyTypesMap).length > 0;
        
        if (hasTypes) {
          // Transform immediately if types are available
          ...
          // Store raw data and transform later when types arrive
          rawExclusiveProperties.current = exclusiveDb;
          // Transform with empty types map for now (will retransform when types load)
          const transformed = exclusiveDb.map(p => transformPropertyForCard(p, {}, tCommon));
          setExclusiveProperties(transformed);
        }
        ...
        // On error, only clear if we don't have cached data to show
    ...
    // Load exclusive properties immediately (no waiting)
    loadExclusive();
  ...
  // Load featured properties - with stale-while-revalidate and parallel loading (no delay)
  useEffect(() => {
    const hasCachedData = featuredProperties.length > 0;
    const isStale = needsRefresh;
    ...
    if (hasCachedData) {
      ...
      // If data is stale, fetch fresh data in background (don't show loading)
      if (isStale) {
        console.log('Featured properties cache is stale, fetching fresh data in background...');
        // Continue to fetch in background
      } else {
        // Data is fresh, no need to fetch
        return;
    ...
    const loadFeatured = async () => {
      try {
        // Only show loading if we don't have cached data
        if (!hasCachedData) {
          console.log('Loading featured properties (parallel with exclusive)...');
          setFeaturedLoading(true);
        } else {
          console.log('Refreshing featured properties in background...');
        }
        ...
        if (hasTypes) {
          ...
          // Store raw data and transform later when types arrive
          rawFeaturedProperties.current = featuredDb;
          // Transform with empty types map for now (will retransform when types load)
          const transformed = featuredDb.map(p => transformPropertyForCard(p, {}, tCommon));
          setFeaturedProperties(transformed);
        }
    ...
    // Load featured properties immediately (parallel with exclusive, no delay)
    loadFeatured();

    return () => {
      mounted = false;
    };
  }, [featuredPage, needsRefresh, propertyTypesMap]); // Depend on featuredPage, needsRefresh, and propertyTypesMap
